...
namespace ns3 {
  ...
  GeoNet::sendSHB (GNDataRequest_t dataRequest,GNCommonHeader commonHeader, GNBasicHeader basicHeader,GNlpv_t longPV)
  {
    ...
    Gnmetadataheader.SetTst (longPV.TST);
    ...
    //std::cout <<"GN Metadata-> ID: " << Gnmetadataheader.GetStationID ()<<"; Lat: " << Gnmetadataheader.GetLat () <<"; Lon: " << Gnmetadataheader.GetLon ()
    //         <<"; TST: " << Gnmetadataheader.GetTst ()<< std::endl;
    /* ---------------------------------------------- */
  ...
  GeoNet::receiveGN(Ptr<Socket> socket)
  {
    ...
    /* [TBR] EXPERIMENTAL, FOR S-LDM DEVELOPMENT ONLY */
    GNMetadataHeader Gnmetadataheader;
    dataIndication.data->RemoveHeader (Gnmetadataheader,20);
    std::cout << "GNMetadataHeader --- Gn-timestamp: " << (unsigned long)(Gnmetadataheader.GetTst ())
              << ";Lat: " << (double) (Gnmetadataheader.GetLat ())/DOT_ONE_MICRO << "; Lon: " << (double) (Gnmetadataheader.GetLon ())/DOT_ONE_MICRO
