...
void AMQP_client::on_container_start(proton::container &c) {
    ...
    std::cout << "innan open_sender" << std::endl;

    c.connect(m_url,co);
    std::cout << "efter open_sender!!!" << std::endl;

}
// In AMQP_client::on_container_start
// void AMQP_client::on_container_start(proton::container &c) {
//     proton::connection_options co;
//     co.sasl_allowed_mechs("PLAIN"); // Specify PLAIN SASL mechanism
//     c.open_receiver(m_url, co);
// }
...
AMQP_client::AMQP_client() {
    ...
    m_url = "amqp://admin:admin@127.0.0.1:5672";
