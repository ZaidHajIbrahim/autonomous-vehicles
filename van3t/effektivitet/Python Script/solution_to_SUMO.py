...
def load_graphml_nodes_and_edges(graphml_file):
    ...
    for node in root.findall(".//graphml:node", namespace):
        ...
        coords_element = node.find("graphml:data[@key='key0']", namespace)
        if coords_element is not None and coords_element.text:
            x, y = map(float, coords_element.text.split(","))
...
if __name__ == "__main__":
    # File paths
    solution_file = "van3t/effektivitet/Continuous-CBS/Continuous-CBS/Examples/output_tasks_log.xml"  # Path to the solver solution file
    graphml_file = "van3t/effektivitet/Continuous-CBS/Continuous-CBS/Examples/output.xml"           # Path to the GraphML file
