...
/*
    ...
    */
    if (!m_netns_name.empty())
{
    std::string prefix;

    if (geteuid() == 0)
    {
        // We are already root, no need to use sudo.
        NS_LOG_INFO("SUMO will be launched in network namespace (root): " << m_netns_name);
        prefix = "ip netns exec ";
    }
    else
    {
        // Not root: rely on sudo (user must have permission / password).
        NS_LOG_WARN("Not running as root. Using sudo to launch SUMO in network namespace: " << m_netns_name);
        prefix = "sudo ip netns exec ";
    }

    m_sumoCommand = prefix + m_netns_name + " " + m_sumoCommand;
}
